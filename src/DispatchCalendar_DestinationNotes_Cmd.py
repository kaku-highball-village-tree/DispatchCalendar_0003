...
def build_step0010_numbered_calendar_title(i_number: int, title_text: str) -> str:
    """Build a numbered Google Calendar title for a sorted step0010 row."""
    return f"{i_number:02d} {title_text}"


def build_numbered_step0010_calendar_records(
    list_step0010_calendar_records: list[dict[str, str]],
) -> list[dict[str, str]]:
    """Return records sorted by date/vehicle type with numbered Google Calendar summaries."""
    dict_records_by_date: dict[str, list[dict[str, str]]] = {}
    for i_record_index, record in enumerate(list_step0010_calendar_records):
        sortable_record = dict(record)
        sortable_record["_source_index"] = str(i_record_index)
        dict_records_by_date.setdefault(record["date"], []).append(sortable_record)

    list_numbered_records: list[dict[str, str]] = []
    for work_date_text in sorted(dict_records_by_date.keys()):
        list_sorted_daily_records = sorted(
            dict_records_by_date[work_date_text],
            key=lambda record: (
                get_step0010_title_vehicle_sort_index(record.get("summary", "")),
                int(record.get("_source_index", "0")),
            ),
        )
        for i_daily_number, record in enumerate(list_sorted_daily_records, start=1):
            numbered_record = {key: value for key, value in record.items() if key != "_source_index"}
            numbered_record["summary"] = build_step0010_numbered_calendar_title(i_daily_number, record["summary"])
            work_date = datetime.strptime(numbered_record["date"], "%Y-%m-%d")
            numbered_record["key"] = build_step0010_calendar_record_key(
                work_date,
                numbered_record["summary"],
                i_daily_number,
            )
            list_numbered_records.append(numbered_record)

    return list_numbered_records
...
def create_google_calendar_events_from_step0010_tsv_files(list_step0010_tsv_file_paths: list[Path]) -> tuple[int, int]:
    ...
    total_skip_count = 0
    for record in build_numbered_step0010_calendar_records(list_current_records):
        try:
...
def get_step0010_sync_month_range(list_step0010_tsv_file_paths: list[Path]) -> tuple[datetime, datetime]:
    """Return the target month range for step0010 Google Calendar sync."""
    sample_step0010_tsv_file_path = sorted(list_step0010_tsv_file_paths, key=lambda file_path: file_path.name)[0]
    target_date = parse_step0010_daily_date(sample_step0010_tsv_file_path)
    month_start_date = datetime(target_date.year, target_date.month, 1)
    if target_date.month == 12:
        next_month_start_date = datetime(target_date.year + 1, 1, 1)
    else:
        next_month_start_date = datetime(target_date.year, target_date.month + 1, 1)

    return month_start_date, next_month_start_date


def get_google_calendar_event_work_date_text(event_item: dict[str, object]) -> str:
    """Return a YYYY-MM-DD work date string from a Google Calendar event item."""
    start_value = event_item.get("start", {})
    if not isinstance(start_value, dict):
        return ""

    start_date = str(start_value.get("date", "")).strip()
    if start_date != "":
        return start_date

    start_date_time = str(start_value.get("dateTime", "")).strip()
    if len(start_date_time) >= 10:
        return start_date_time[:10]

    return ""


def list_google_calendar_events_for_step0010_month(
    google_calendar_service,
    calendar_id: str,
    month_start_date: datetime,
    next_month_start_date: datetime,
) -> list[dict[str, object]]:
    """List current Google Calendar events in the step0010 target month."""
    list_event_items: list[dict[str, object]] = []
    page_token: str | None = None
    time_min = month_start_date.isoformat() + "+09:00"
    time_max = next_month_start_date.isoformat() + "+09:00"

    while True:
        request = google_calendar_service.events().list(
        ...
        for event_item in response.get("items", []):
            if isinstance(event_item, dict):
                list_event_items.append(event_item)

        page_token = response.get("nextPageToken")
        if not page_token:
            break

    return list_event_items


def build_google_calendar_event_records_from_event_items(
    list_event_items: list[dict[str, object]],
) -> list[dict[str, str]]:
    """Build comparable records from current Google Calendar event items."""
    list_google_calendar_event_records: list[dict[str, str]] = []
    dict_occurrence_count_by_date_and_title: dict[tuple[str, str], int] = {}

    for event_item in sorted(
        list_event_items,
        key=lambda item: (
            get_google_calendar_event_work_date_text(item),
            str(item.get("summary", "")).strip(),
            str(item.get("id", "")).strip(),
        ),
    ):
        work_date_text = get_google_calendar_event_work_date_text(event_item)
        summary_text = str(event_item.get("summary", "")).strip()
        event_id = str(event_item.get("id", "")).strip()
        if work_date_text == "" or summary_text == "" or event_id == "":
            continue

        occurrence_key = (work_date_text, summary_text)
        occurrence_index = dict_occurrence_count_by_date_and_title.get(occurrence_key, 0) + 1
        dict_occurrence_count_by_date_and_title[occurrence_key] = occurrence_index
        work_date = datetime.strptime(work_date_text, "%Y-%m-%d")
        record = {
            "key": build_step0010_calendar_record_key(work_date, summary_text, occurrence_index),
            "date": work_date_text,
            "summary": summary_text,
            "event_id": event_id,
        }
        color_id = str(event_item.get("colorId", "")).strip()
        if color_id != "":
            record["colorId"] = color_id
        list_google_calendar_event_records.append(record)

    return list_google_calendar_event_records
...
def sync_google_calendar_events_from_step0010_tsv_files(list_step0010_tsv_file_paths: list[Path]) -> tuple[int, int]:
    ...
    month_start_date, next_month_start_date = get_step0010_sync_month_range(list_step0010_daily_tsv_file_paths)
    list_google_calendar_event_items = list_google_calendar_events_for_step0010_month(
        google_calendar_service,
        calendar_id,
        month_start_date,
    ...
    list_numbered_current_records = build_numbered_step0010_calendar_records(list_current_records)
    dict_numbered_current_records_by_date: dict[str, list[dict[str, str]]] = {}
    dict_google_calendar_records_by_date: dict[str, list[dict[str, str]]] = {}

    for record in list_numbered_current_records:
        dict_numbered_current_records_by_date.setdefault(record["date"], []).append(record)
    ...
    list_target_date_texts = sorted(set(dict_numbered_current_records_by_date.keys()) | set(dict_google_calendar_records_by_date.keys()))
    ...
    for work_date_text in list_target_date_texts:
        ...
        for record in dict_numbered_current_records_by_date.get(work_date_text, []):
            try:
                ...
                list_sync_error_lines.append(
                    f"sync=create, reason={exception}, date={record.get('date', '')}, "
                    f"summary={record.get('summary', '')}"
                )
...
def delete_google_calendar_events_from_step0010_tsv_files(list_step0010_tsv_file_paths: list[Path]) -> tuple[int, int]:
    ...
    google_calendar_service = build("calendar", "v3", credentials=get_google_credentials())
    calendar_id = get_google_calendar_id()
    dict_vehicle_type_to_color_id, _ = read_google_calendar_color_settings()
    list_current_records, dict_error_lines_by_path = build_current_step0010_calendar_event_records(
        list_step0010_daily_tsv_file_paths,
        dict_vehicle_type_to_color_id,
    )
    ...
    total_skip_count = 0
    list_delete_error_lines: list[str] = []
    for record in build_numbered_step0010_calendar_records(list_current_records):
        try:
            if delete_google_calendar_event_from_step0010_record(google_calendar_service, calendar_id, record):
                ...
                total_skip_count += 1
                list_delete_error_lines.append(
                    f"sync=delete, reason=matching event not found, date={record.get('date', '')}, "
                    f"summary={record.get('summary', '')}"
                )
        except Exception as exception:
            total_skip_count += 1
    ...
    if len(list_delete_error_lines) > 0:
        first_step0010_tsv_file_path = sorted(list_step0010_daily_tsv_file_paths, key=lambda file_path: file_path.name)[0]
        write_step0010_registration_error_file(first_step0010_tsv_file_path, list_delete_error_lines)
